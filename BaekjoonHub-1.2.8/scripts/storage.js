...
chrome.storage.local.get('isSync', (data) => {
  keys = ['BaekjoonHub_token', 'BaekjoonHub_username', 'pipe_baekjoonhub', 'stats', 'BaekjoonHub_hook', 'mode_type', 'BaekjoonHub_BaseDir'];
  if (!data || !data.isSync) {
    keys.forEach((key) => {
...
async function getBaseDir() {
  const baseDir = await getObjectFromLocalStorage('BaekjoonHub_BaseDir');
  if (typeof baseDir !== 'string') return '';
  return baseDir.replace(/^\/+|\/+$/g, '').trim();
}
...
async function getDirNameByOrgOption(dirName, language) {
  ...
  return baseDir ? `${baseDir}/${dirName}` : dirName;
}
