...
$('#hook_button').on('click', () => {
  ...
  chrome.storage.local.set({ BaekjoonHub_BaseDir: base_dir }, () => {
    console.log(`Set BaseDir: ${base_dir}`);
  });
});
...
chrome.storage.local.get(['BaekjoonHub_OrgOption', 'BaekjoonHub_BaseDir'], (data) => {
  if (data.BaekjoonHub_OrgOption) {
    $('#org_option').val(data.BaekjoonHub_OrgOption);
  }
  if (typeof data.BaekjoonHub_BaseDir === 'string') {
    $('#base_dir').val(data.BaekjoonHub_BaseDir);
  }
});
